...
vSongBook requires Qt as well as SQLite. For more information on Qt
...
in the main directory. This will generate the vSongBook (or
`vSongBook.exe`, or `vSongBook.app`) application in the src subdirectory.
...
$ git clone https://github.com/vSongBook/vSongBook4PC
$ cd vSongBook
$ mkdir build
$ cd build
...
This should complete without errors, giving you a binary file called 'vSongBook'.
...
This should complete without errors, and `vSongBook` should now be launch-able from the command line.
...
This is incredibly easy using [Homebrew](http://brew.sh).  Just run this command:
    ...
    $ brew install vSongBook

And you're done.  A "vSongBook" command should now be available in your PATH,
...
Then it's just a matter of getting the source:
    ...
    $ git clone https://github.com/vSongBook/vSongBook4PC.git
...
An icon for "vSongBook" should now be in your main OSX Applications
list, ready to launch.

**Note 2** - There have been occasional [reports of compilation problems on OSX
10.9](https://github.com/vSongBook/vSongBook4PC/issues/38), with the
...
Install Qt and SQLite as instructed in the previous section and clone the repo to a directory (it's `vSongBook` as usual).
...
&nbsp; &nbsp; https://github.com/vSongBook/vSongBook4PC/wiki/Setting-up-a-Win64-development-environment-for-DB4S
...
Now cd into your vSongBook source directory and create a build directory
...
When built with SQLCipher support, vSongBook will allow you to open
...
vSongBook has unit tests in the "src/tests" subdirectory.

### Build the unit tests

The unit tests are enabled using the cmake variable `ENABLE_TESTING`;
it can be passed when running `cmake` to configure vSongBook,
