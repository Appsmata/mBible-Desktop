...
void AppHome::SearchBible(QString SearchStr)
{
	...
	//ui->TxtContent->setPlainText(AsUtils::BOOK_SEARCH_SQL(SearchStr, SearchOT, SearchNT));
...
void AppHome::OpenReading()
{
	...
	if (rc == SQLITE_OK)
	{
		...
		//ui->TxtContent->setPlainText(AsUtils::VERSE_READING_SQL(book, chapter, language));
		rc = sqlite3_get_table(db, sqlQuery2, &qryResult, &row, &col, &err_msg);
